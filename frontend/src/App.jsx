...
function Login({onLogin,theme,setTheme}){const[email,setEmail]=useState('');const[password,setPassword]=useState('');const[error,setError]=useState('');const[busy,setBusy]=useState(false);return <div className="login-shell"><Aurora/><button className="floating-theme" onClick={()=>setTheme(theme==='dark'?'light':'dark')}>{theme==='dark'?<Sun/>:<Moon/>}</button><div className="login-visual"><Brand/><div><h1>Integration Hub<br/>Admin Portal.</h1><p>Central control for job aggregation and verification.</p></div><div className="login-proof"><ShieldCheck/><span><strong>Restricted access</strong><small>Authorized personnel only.</small></span></div></div><form className="login-card" onSubmit={async e=>{e.preventDefault();setError('');setBusy(true);try{onLogin(await api.login(email,password))}catch(err){setError(err.message)}finally{setBusy(false)}}}><Brand/><div><h2>Welcome back</h2><p>Sign in to TalentShift Integration Hub.</p></div>{error&&<div className="error-banner" role="alert">{error}</div>}<label>Email address<input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="admin@example.com" autoComplete="email" required/></label><label>Password<input type="password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="Enter your password" required/></label><button className="btn btn-primary" disabled={busy}>{busy?'Signing in…':'Sign in'}</button></form></div>}

function Page({id,user}){switch(id){
  case'operations':return <Operations/>;case'cleanup':return <DatabaseCleanup/>;case'categories':return <SourceCategories/>;case'approvals':return <AiSourceApprovals/>;case'jobs':return <Jobs/>;default:return <Operations/>}}
...
// Candidate components removed from Integration Hub, but Jobs preview restored for admins

function Jobs(){const[q,setQ]=useState('');const[locationValue,setLocation]=useState('');const[page,setPage]=useState(0);const params=useMemo(()=>({keyword:q,location:locationValue,remote:false,page,size:100}),[q,locationValue,page]);const result=useLoad(()=>api.fetchJobs(params),[q,locationValue,page]);const[counts,setCounts]=useState({onsite:0});useEffect(()=>{Promise.all([api.fetchJobs({remote:false,size:1})]).then(([on])=>setCounts({onsite:on.total})).catch(()=>{});},[]);useEffect(()=>setPage(0),[q,locationValue]);const totalPages=result.data?.total?Math.ceil(result.data.total/100):0;return <><PageHead title="Job Database Preview" subtitle="Search and browse all verified active jobs currently held in the Hub."/><div className="search-rail"><Search/><input value={q} onChange={e=>setQ(e.target.value)} placeholder="Search roles or skills"/><input value={locationValue} onChange={e=>setLocation(e.target.value)} placeholder="Location"/></div><section className="panel"><State {...result}><JobRows jobs={array(result.data)}/>{totalPages>1&&<div className="pagination"><button className="btn" disabled={page===0} onClick={()=>setPage(p=>p-1)}>Previous</button><span>Page {page+1} of {totalPages}</span><button className="btn" disabled={page>=totalPages-1} onClick={()=>setPage(p=>p+1)}>Next</button></div>}</State></section></>}
...
return <><PageHead title="Collection control center" subtitle="Private administrator controls for collection, verification, discovery, and database maintenance." actions={<><button className="btn" onClick={()=>run(api.recheckSources,'Rechecking sources')}><RefreshCw/> Recheck sources</button><button className="btn btn-primary" onClick={()=>run(api.collectJobs,'Collecting')}><Play/> Run collection</button></>}/>{action&&<div className="notice">{action}… automatic schedules remain active.</div>}{operationError&&<div className="error-banner" role="alert">{operationError}</div>}<State {...status}><div className="metrics-grid"><Stat label="Active jobs" value={d.activeJobs} detail="Visible to candidates"/><Stat label="Expired jobs" value={d.expiredJobs} detail="Hidden from search"/><Stat label="Total sources" value={d.totalSources}/><Stat label="Healthy sources" value={d.healthySources}/><Stat label="Last collection" value={date(d.lastCollection)}/></div></State><section className="panel"><div className="section-title"><div><h2>Source Performance Breakdown</h2><p>Detailed performance per source showing total jobs collected.</p></div></div><State {...performance}><DataTable data={array(performance.data)}/></State></section></>}
