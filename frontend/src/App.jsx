...
function App(){
  ...
  const acceptUser=u=>{setUser(u);sessionStorage.setItem('talentshift_authenticated','true');const allowed=(u.role==='ADMIN'?adminNav:candidateNav).some(([id])=>id===(location.hash.slice(1)||''));if(!allowed)location.hash=u.role==='ADMIN'?'operations':'dashboard'};
  useEffect(()=>{api.getMe().then(acceptUser).catch(()=>setUser(null)).finally(()=>setChecking(false));},[]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const visibleNav=isAdmin?adminNav:candidateNav;
  if(!visibleNav.some(([id])=>id===page))location.hash=isAdmin?'operations':'dashboard';
  const navigate=id=>{if(location.hash!==`#${id}`)location.hash=id;else setPage(id)};
  return <div id="root"><Aurora/><aside className="sidebar">
    <Brand/><div className="nav-scroll">{isAdmin?<NavGroup label="Administration" items={adminNav} page={page} navigate={navigate}/>:<NavGroup label="Candidate workspace" items={candidateNav} page={page} navigate={navigate}/>}</div>
...
function Login({onLogin,theme,setTheme}){const[mode,setMode]=useState('login');const[fullName,setFullName]=useState('');const[email,setEmail]=useState('');const[password,setPassword]=useState('');const[confirmPassword,setConfirmPassword]=useState('');const[error,setError]=useState('');const[busy,setBusy]=useState(false);const creating=mode==='register';const switchMode=()=>{setMode(creating?'login':'register');setError('');setPassword('');setConfirmPassword('')};return <div className="login-shell"><Aurora/><button className="floating-theme" onClick={()=>setTheme(theme==='dark'?'light':'dark')} aria-label={theme==='dark'?'Use light mode':'Use dark mode'}>{theme==='dark'?<Sun/>:<Moon/>}</button><div className="login-visual"><Brand/><div><h1>{creating?'Start your next chapter.':'Build a clearer path'}<br/>{creating?'Your career, organized.':'to your next role.'}</h1><p>{creating?'Create your candidate account to save roles, track applications, and manage your career journey.':'Verified opportunities, direct application routes, and one place to manage your career.'}</p></div><div className="login-proof"><ShieldCheck/><span><strong>Public, verifiable sources</strong><small>Designed around traceability and direct job links.</small></span></div></div><form className="login-card" onSubmit={async e=>{e.preventDefault();setError('');if(creating&&password!==confirmPassword){setError('Passwords do not match.');return}setBusy(true);try{onLogin(creating?await api.register(fullName,email,password):await api.login(email,password))}catch(err){setError(err.message)}finally{setBusy(false)}}}><Brand/><div><h2>{creating?'Create your account':'Welcome back'}</h2><p>{creating?'Join TalentShift as a candidate.':'Sign in to continue to TalentShift.'}</p></div>{error&&<div className="error-banner" role="alert">{error}</div>}{creating&&<label>Full name<input value={fullName} onChange={e=>setFullName(e.target.value)} placeholder="Enter your full name" minLength="2" maxLength="120" autoComplete="name" required/></label>}<label>Email address<input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="you@example.com" autoComplete="email" required/></label><label>Password<input type="password" value={password} onChange={e=>setPassword(e.target.value)} placeholder={creating?'Create a secure password':'Enter your password'} minLength={creating?12:undefined} maxLength="128" pattern={creating?'(?=.*[a-z])(?=.*[A-Z])(?=.*\\d).+':undefined} title={creating?'Use at least 12 characters with uppercase, lowercase, and a number.':undefined} autoComplete={creating?'new-password':'current-password'} required/></label>{creating&&<><small className="password-hint">At least 12 characters with uppercase, lowercase, and a number.</small><label>Confirm password<input type="password" value={confirmPassword} onChange={e=>setConfirmPassword(e.target.value)} placeholder="Confirm your password" minLength="12" maxLength="128" autoComplete="new-password" required/></label></>}<button className="btn btn-primary" disabled={busy}>{busy?(creating?'Creating account…':'Signing in…'):(creating?'Create account':'Sign in')}</button><div className="auth-switch"><span>{creating?'Already have an account?':'New to TalentShift?'}</span><button type="button" onClick={switchMode}>{creating?'Sign in':'Create an account'}</button></div></form></div>}
